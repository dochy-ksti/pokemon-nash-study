#!/usr/bin/env bash
# 敵混合 自己対戦比率 r=0.45 掃引ドライバ (K1r045 funnel → rate)。切断分離実行。
set -u
cd "$(dirname "$0")/.." || exit 1
...
pkill -f "ckpt_tournament.py funnel --tag K1r045" 2>/dev/null
sleep 2
echo "[driver] make build"
make build > "$LOGDIR/ratio_r045_build.log" 2>&1 || { echo "[driver] build failed"; exit 1; }
...
run_funnel K1r045 0.45 || { echo "[driver] K1r045 failed"; exit 1; }

echo "[driver] === rate (RC64+K1+K1r06+K1r04+K1r03+K1r045) $(date -Is) ==="
uv run python scripts/ckpt_tournament.py rate \
  ...
  --n-per-side 512 --num-games 256 --stage 3b --random --crit \
  > "$LOGDIR/ratio_r045_rate.log" 2>&1
