#!/usr/bin/env bash
# 敵混合 自己対戦比率 r=0.35 掃引ドライバ (K1r035 funnel → rate)。切断分離実行。
set -u
cd "$(dirname "$0")/.." || exit 1
...
pkill -f "ckpt_tournament.py funnel --tag K1r035" 2>/dev/null
sleep 2
echo "[driver] make build"
make build > "$LOGDIR/ratio_r035_build.log" 2>&1 || { echo "[driver] build failed"; exit 1; }
...
run_funnel K1r035 0.35 || { echo "[driver] K1r035 failed"; exit 1; }

echo "[driver] === rate (RC64+K1+K1r06+K1r04+K1r03+K1r045+K1r035) $(date -Is) ==="
uv run python scripts/ckpt_tournament.py rate \
  --funnel-json "$TDIR/RC64_finalists.json" "$TDIR/K1_finalists.json" \
                ...
                "$TDIR/K1r035_finalists.json" \
  --n-per-side 512 --num-games 256 --stage 3b --random --crit \
  > "$LOGDIR/ratio_r035_rate.log" 2>&1
