#!/usr/bin/env bash
# 敵混合 自己対戦比率掃引ドライバ (K1r06 funnel → rate)。
# CLI から切り離して実行できるよう自己完結。既存 finalists.json があれば skip。
# 使い方 (SSH/Windows 切断後も継続):
#   cd poke-ai3-python
#   setsid nohup bash scripts/run_ratio_sweep.sh > "$LOGDIR/ratio_sweep_driver.log" 2>&1 &
set -u
...
# 既存の途中 K1r06 funnel があれば止める (同名ファイル二重書き防止)。
pkill -f "ckpt_tournament.py funnel --tag K1r06" 2>/dev/null
sleep 2

echo "[driver] make build"
make build > "$LOGDIR/ratio_sweep_build.log" 2>&1 || { echo "[driver] build failed"; exit 1; }
...
run_funnel K1r06 0.6 || { echo "[driver] K1r06 failed"; exit 1; }

echo "[driver] === rate (RC64+K1+K1r06) $(date -Is) ==="
uv run python scripts/ckpt_tournament.py rate \
  --funnel-json "$TDIR/RC64_finalists.json" "$TDIR/K1_finalists.json" \
                "$TDIR/K1r06_finalists.json" \
  --n-per-side 512 --num-games 256 --stage 3b --random --crit \
  > "$LOGDIR/ratio_sweep_rate.log" 2>&1
