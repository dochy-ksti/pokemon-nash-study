#!/usr/bin/env bash
# 敵混合 自己対戦比率 r=0.4 掃引ドライバ (K1r04 funnel → rate)。
# 既存 finalists.json があれば skip。切断分離実行:
#   cd poke-ai3-python
#   setsid nohup bash scripts/run_ratio_sweep_r04.sh > "$LOGDIR/ratio_r04_driver.log" 2>&1 &
set -u
...
pkill -f "ckpt_tournament.py funnel --tag K1r04" 2>/dev/null
sleep 2

echo "[driver] make build"
make build > "$LOGDIR/ratio_r04_build.log" 2>&1 || { echo "[driver] build failed"; exit 1; }
...
run_funnel K1r04 0.4 || { echo "[driver] K1r04 failed"; exit 1; }

echo "[driver] === rate (RC64+K1+K1r06+K1r04) $(date -Is) ==="
