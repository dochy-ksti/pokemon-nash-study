#!/usr/bin/env bash
# 敵混合 自己対戦比率 r=0.3 掃引ドライバ (K1r03 funnel → rate)。切断分離実行。
set -u
cd "$(dirname "$0")/.." || exit 1
...
pkill -f "ckpt_tournament.py funnel --tag K1r03" 2>/dev/null
sleep 2
echo "[driver] make build"
make build > "$LOGDIR/ratio_r03_build.log" 2>&1 || { echo "[driver] build failed"; exit 1; }
...
run_funnel K1r03 0.3 || { echo "[driver] K1r03 failed"; exit 1; }

echo "[driver] === rate (RC64+K1+K1r06+K1r04+K1r03) $(date -Is) ==="
uv run python scripts/ckpt_tournament.py rate \
  --funnel-json "$TDIR/RC64_finalists.json" "$TDIR/K1_finalists.json" \
                ...
                "$TDIR/K1r03_finalists.json" \
  --n-per-side 512 --num-games 256 --stage 3b --random --crit \
  > "$LOGDIR/ratio_r03_rate.log" 2>&1
